...
constexpr quint32 kCodeWindow = 0x100;
...
enum code_column { col_marker = 0, col_address, col_opcode, col_text };
...
void debugger_panel::build_ui() {
    ...
    code_view_->setColumnCount(4);
    code_view_->setHeaderLabels({QString(), tr("Address"), tr("Opcode"), tr("Disassembly")});
    ...
    stack_view_->setColumnCount(3);
    stack_view_->setHeaderLabels({tr("#"), tr("Address"), tr("Frame")});
...
void debugger_panel::refresh_actions() {
    ...
    resume_btn_->setEnabled(have_pc);
    halt_btn_->setEnabled(live && thread_id_ != 0);
    ...
    if (running_) {
        ...
        status_->setText(tr("stopped at %1  thread %2").arg(hex64(pc_), hex_short(thread_id_)));
    } else if (have_regs_) {
...
void debugger_panel::on_memory_ready(quint64 address, QByteArray bytes) {
    if (stack_pending_ && address == stack_base_) {
        ...
        show_callstack(bytes);
        return;
    }
    if (memory_pending_ && address == memory_base_) {
...
void debugger_panel::show_callstack(const QByteArray& stack) {
    ...
    const auto frames = opentm::tm_core::walk_stack(view, stack_base_, pc_, regs_.gpr[1]);
...
void debugger_panel::stack_frame_activated() {
    ...
    if (ok && address != 0) request_disassembly(address & ~static_cast<quint64>(kCodeWindow - 1));
}
...
void debugger_panel::redraw_disassembly() {
    ...
    for (const auto& insn : insns_) {
        ...
        if (stopped_ && insn.address == pc_)     marker += QStringLiteral(">");
        item->setText(col_marker, marker);

        if (stopped_ && insn.address == pc_) {
            code_view_->setCurrentItem(item);
        }
    }
    refresh_actions();
}
...
void debugger_panel::on_thread_stopped(quint64 thread_id, quint64 address, quint32 reason) {
    ...
    request_disassembly(address & ~static_cast<quint64>(kCodeWindow - 1));
    refresh_actions();
}
...
void debugger_panel::on_registers_ready(quint64 thread_id, opentm::tm_core::dbgp::ppu_registers regs) {
    ...
    if (regs.pc > 0xffffffffull) {
        emit log_message(tr("!! debugger: thread %1 returned pc=%2, which is not an address. It is probably not halted.").arg(hex_short(thread_id), hex64(regs.pc)));
        have_regs_ = false;
        refresh_actions();
        return;
    }
    ...
    if (regs.pc != 0) {
        ...
        if (moved || !have_it) {
            request_disassembly(pc_ & ~static_cast<quint64>(kCodeWindow - 1));
            return;
        }
    }
    redraw_disassembly();
